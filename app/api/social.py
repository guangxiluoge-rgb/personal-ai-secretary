...
def circle_list(db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    ...
    return [{"id": r.id, "name": r.name, "description": r.description, "visibility": r.visibility, "member_count": db.query(SocialCircleMember).filter(SocialCircleMember.circle_id == r.id, SocialCircleMember.status == "active").count(), "updated_at": r.updated_at} for r in rows]
...
def people_search(q: str = "", db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    rows = db.query(Person).filter(Person.user_id == user_id).order_by(Person.updated_at.desc()).limit(100).all() if not q.strip() else search_circle_people(db, user_id, q)
...
def person_profile(person_id: int, db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id)):
    ...
    events = db.query(RelationshipEvent).filter(RelationshipEvent.user_id == user_id, RelationshipEvent.person_id == person_id).order_by(RelationshipEvent.created_at.desc()).limit(80).all()
    risks = db.query(RiskAlert).filter(RiskAlert.user_id == user_id, RiskAlert.person_id == person_id, RiskAlert.status == "open").order_by(RiskAlert.created_at.desc()).limit(30).all()
    tasks = [t for t in list_open_tasks(db, user_id, 200) if t.person_id == person_id][:30]
    ...
    normal_events = sum(1 for e in events if e.risk_level == "normal")
    score = max(0, min(100, 45 + min(30, len(recent) * 5) + min(15, normal_events * 2) - min(35, len(risks) * 12) - min(20, len(tasks) * 3)))
    ...
    if not events:
        ...
        summary = "近期有持续沟通记录，当前没有关联的未处理风险；继续积累事实后再观察变化。"
    if risks:
    ...
    return {"person":{"id":person.id,"name":person.name,"relationship_type":person.relationship_type,"notes":person.notes,"importance":person.importance},"relationship":{"score":score,"summary":summary,"basis":basis},"risk_count":len(risks),"open_tasks":[{"id":t.id,"title":t.title,"priority":t.priority,"due_at":t.due_at} for t in tasks],"timeline":[{"id":e.id,"event_type":e.event_type,"summary":e.summary,"risk_level":e.risk_level,"created_at":e.created_at} for e in events],"recent_meetings":[{"id":m.id,"title":m.title,"summary":m.summary[:500]} for m in meetings if person.name in m.title or person.name in m.summary][:10],"decision_advice":advice}
